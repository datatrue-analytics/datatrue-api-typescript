...
namespace DataTrue {
  ...
  export const _run = function _run(email_users: number[] = [], resourceTypeRun: string, resourceID: number): number {
    ...
    const request = DataTrue._makeRequest("post", uri, JSON.stringify({
      "test_run": {
   ...
   */
  export const _progress = function _progress(jobID: number): DataTrue.JobStatus {
    const uri = [
      DataTrue.apiEndpoint,
      "ci_api",
      "test_runs",
      "progress",
      `${jobID}?api_key=${DataTrue.ciToken}`,
    ].join("/");
